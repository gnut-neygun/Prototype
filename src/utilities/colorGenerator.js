/**
 * Credits to https://stackoverflow.com/questions/10014271/generate-random-color-distinguishable-to-humans
 * @param number
 * @returns string[] a string array of #hex_value
 */
...
//if we've passed preloaded colors and they're in hex format
    if (typeof (arguments[1]) != 'undefined' && arguments[1].constructor === Array && arguments[1][0] && arguments[1][0].constructor != Array) {
        for (let i = 0; i < arguments[1].length; i++) { //for all the passed colors
    ...
    number += loadedColors.length//reset number to include the colors already passed
    let lastLoadedReduction = Math.floor(Math.random() * 3) //set a random value to be the first to decrease
    function rgbToHSL(rgb) {//converts [r,g,b] into [h,s,l]
    ...
    function hslToRGB(hsl) {//converts [h,s,l] into [r,g,b]
        let h = hsl[0], s = hsl[1], l = hsl[2], c = (1 - Math.abs(2 * l - 1)) * s,
    ...
    function shiftHue(rgb, degree) {//shifts [r,g,b] by a number of degrees
    ...
    function fixDifference(color) {//recursively asserts that the current color is distinctive
        ...
        for (let i = 0; i < loadedColors.length; i++) { //for all the colors we've generated so far
            const difference = loadedColors[i].map(function (value, index) { //for each value (red,green,blue)
    ...
    function color() { //generate a random color
    ...
    for (let i = loadedColors.length; i < number; i++) { //Start with our predefined colors or 0, and generate the correct number of colors.
